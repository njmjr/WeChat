...
namespace Dapper.Contrib.Extensions
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static IEnumerable<T> Find<T>(this IDbConnection connection, string query, object param, IDbTransaction transaction = null,
            int? commandTimeout = null) where T : class
        {
            ...
            var result = connection.Query<T>(sql, param, transaction, commandTimeout: commandTimeout);
            return result;
        }
        ...
        public static bool Delete<T>(this IDbConnection connection, dynamic primaryId, IDbTransaction transaction = null,bool isTrimKey = false, int? commandTimeout = null) where T : class
        {  
            var type = typeof(T);
            ...
            if (keyProperties.Count  != 1)
                throw new ArgumentException("Entity must have at only one [Key] property");
            ...
            sb.AppendFormat("delete from {0} where ", name);
            var dynParms = new DynamicParameters();
            dynParms.Add(":id", primaryId);
            for (var i = 0; i < keyProperties.Count(); i++)
            {
                var property = keyProperties.ElementAt(i);
                if (isTrimKey)
                {
                    sb.AppendFormat("TRIM({0}) = TRIM(:id)", property.Name);
                }
                else
                {
                    sb.AppendFormat("{0} = :id", property.Name);
                } 
            }
            var deleted = connection.Execute(sb.ToString(), dynParms, transaction, commandTimeout);
            return deleted > 0;
        }
